import {CreateGroupDTO, JoinGroupDTO, LeaveGroupDTO} from "../controllers/group";
...
export class GroupService {
    public static async createGroup(request: CreateGroupDTO): Promise<OperationResult> {
        ...
        const users = await UserRepository.findBy({ username: In(request.members)})
    ...
    public static async joinGroup(request: JoinGroupDTO): Promise<OperationResult> {
    ...
    public static async leaveGroup(request: LeaveGroupDTO): Promise<OperationResult> {
        const group = await GroupRepository.findOne({where: {name: request.group}, relations: ["members"]})
        if (!group){
            return opFailure(HttpStatusCode.NotFound, `Cannot find group with name ${request.group}`)
        }
